...
const withClappedAnimation = (WrappedComponent) => {
  class WithClappedAnimation extends React.Component {
  ...
  return <WithClappedAnimation />;
};

const MediumClap = ({ animationTimeline }) => {
  const MAXIMUM_CLAPS = 50;
  const [clapState, setClapState] = React.useState(intialValue);
  ...
  const handleClick = () => {
    ...
    setClapState((prevState) => ({
      isClicked: true,
      count: Math.min(prevState.count + 1, MAXIMUM_CLAPS),
      countTotal:
        prevState.count < MAXIMUM_CLAPS
 ...
 */
const Usage = () => {
  const AnimatedClap = withClappedAnimation(MediumClap);
  return AnimatedClap;
};
